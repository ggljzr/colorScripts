...
parser = argparse.ArgumentParser(
    description='Simple script for displaying terminal colors. For Python2.7')
    ...
    for i in range(0, height):
        for color in colors:
            if style == 'both':
                ...
                box = colored(boxChar * (colorWidth / 2),
                              color, attrs=['bold'])
                sys.stdout.write(box)
            else:
...
def textOutput(colors):
    ...
    for background in colors:
        ...
        print ''
        for foreground in colors:
            output = colored(' ' + foreground + ' ',
                             foreground, bckg, attrs=['bold'])
            sys.stdout.write(output)
        print ''
    for foreground in colors:
